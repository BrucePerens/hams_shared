...
def main():
    ...
    for root, dirs, files in os.walk(repo_root):
        if "radae" in dirs:
            dirs.remove("radae")
        # Real bug found 2026-09-12: this list never excluded ".claude" -- this project's own
        # standing convention runs concurrent bug-hunt dispatches in isolated git worktrees
        # under .claude/worktrees/<session>/ inside the repo root, a real git worktree checkout
        # (not an empty directory), so a module's own __init__.py inside another session's
        # in-progress worktree was scanned as if it were real repo content. Same fix already
        # applied the same night to check_shebang.py/check_rabbitmq_pool.py/check_test_tags.py/
        # check_pip_audit.py/check_cargo_deny.py. This is independent of this file's own
        # separately-flagged "possible missing cross-repo scan" open question (see
        # night_shift_todo.md) -- that one needs Bruce's own call on CI/dev-workflow topology;
        # this one is an uncontroversial hygiene fix regardless of how that's resolved.
        dirs[:] = [d for d in dirs if d not in ('target', 'venv', '.venv', 'node_modules', 'daemons', 'test_env', '.git', '.claude')]

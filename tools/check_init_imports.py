...
def main():
    ...
    for root, dirs, files in os.walk(repo_root):
        ...
        # odoo_type_stubs/ is entirely machine-generated by generate_odoo_core_stubs.py, wiped and
        # rebuilt from scratch on every run (604 files as of 2026-08-27) -- it deliberately mirrors
        # each real core-addon source file's path rather than being organized as a normal importable
        # Odoo module, and mypy's plugin reads these stub files directly by path (not via Python's
        # import machinery), so there is no "forgotten import" for this check to catch here, only
        # noise that would immediately go stale the next time the generator regenerates the tree.
        if "odoo_type_stubs" in os.path.relpath(root, repo_root).split(os.sep):
            dirs[:] = []
            continue

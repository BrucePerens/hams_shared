...
def get_headers():
    ...
    if not api_key:
        raise ValueError(
            "JULES_API_KEY environment variable is missing. Please set it."
        )
...
def send_message(session_name, prompt, headers):
    ...
    try:
        ...
        logging.error(
            f"https://jules.app/{session_name}: Failed to send message. Error: {e}"
        )
...
def approve_plan(session_name, headers):
    ...
    try:
        ...
        logging.error(
            f"https://jules.app/{session_name}: Failed to approve plan. Error: {e}"
        )
    ...
    match state:
        ...
        case "PAUSED" | "FAILED" | "STUCK":
            if "Jules encountered an error when cloning the repo" in str(session):
                logging.info(
                    f"https://jules.app/{session_name}: Skipping 'Proceed' because a repo cloning error was detected."
                )
            else:
        ...
        case _:
            logging.warning(
                f"https://jules.app/{session_name} is in unexpected state '{state}'."
            )

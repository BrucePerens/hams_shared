        ...
        for file in files:
            ...
            try:
                ...
                new_content = new_content.replace(
                    "odoo.addons.hams_test.tests.common",
                    "odoo.addons.zero_sudo.tests.common",
                )
                new_content = new_content.replace(
                    "odoo.addons.hams_test.tests.real_transaction",
                    "odoo.addons.zero_sudo.tests.real_transaction",
                )
                new_content = new_content.replace(
                    "@hams_test/js/tour_utils", "@zero_sudo/js/tour_utils"
                )
                new_content = new_content.replace(
                    "hams_test.action_noisy_table", "zero_sudo.action_noisy_table"
                )
                new_content = new_content.replace(
                ...
                if "zero_sudo/tests/" in filepath and filepath.endswith(".py"):
                    ...
                    new_content = new_content.replace(
                        "from odoo.addons.zero_sudo.tests.real_transaction import",

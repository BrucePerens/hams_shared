...
"""
Flags direct `window.fetch = ...` reassignment in *.test.js files.
...
without a mechanical check, since nothing forces these tests to actually run.

This check is deliberately narrow (a single, low-noise regex) rather than a general JS static
analyzer: `window.fetch =` (or `window['fetch'] =`) assigned to anything, excluding `==`/`===`/`!=`/
`!==` comparisons, inside a `*.test.js` file.
"""
                    ...
                    for i, line in enumerate(f, 1):
                        if _ASSIGNMENT_RE.search(line):
                            violations.append(
                                ...
                                "...` reassignment -- this throws under the real @odoo/hoot test "
                                "harness (hoot's mocked window makes window.fetch read-only). Use "
                                "this repo's own mockFetch() helper instead (see "

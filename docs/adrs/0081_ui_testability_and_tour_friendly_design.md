...
> **NOTICE:** The architectural decisions below have been aggregated into a unified developer guide. Please refer to `hams_shared/docs/LLM_WRITING_TOURS.md` for the complete, up-to-date manual on writing UI Tours.
...
### 10. The Website Cookies Bar Out-Ranks Your Modal
Any frontend tour on a `website.layout` page that opens and interacts with its own Bootstrap modal (e.g. `data-bs-target="#someModal"`) is racing against `#website_cookies_bar`, which auto-shows roughly 500ms after page load and is *itself* a Bootstrap `.modal`, injected into `website.layout` after every module-specific modal. Odoo's own tour-engine safety check (`elementIsInModal` in `web_tour/static/src/js/tour_automatic/tour_step_automatic.js`) picks the *last* visible `.modal` in DOM order and requires the click target to be inside that one -- so once the cookies bar appears, every click inside your real modal fails with "It is not allowed to do action on an element that's below a modal," even though the cookies bar has no backdrop and covers nothing on screen. This has already caused real, intermittent tour failures twice (`user_websites/static/tests/tours/violation_report_tour.js`, `ham_events/static/tests/tours/event_issue_tour.js`) before being root-caused.
* Any tour that opens a modal on a frontend page MUST remove the cookies bar before doing so: `document.querySelector('#website_cookies_bar')?.remove();` in an early `body`-triggered step. This is simpler and has no timing dependency compared to waiting for the bar to appear and clicking its close button (`.js_close_popup`) -- there's nothing left to show.
* This is upstream Odoo `web_tour` behavior, not something specific to this codebase -- it will keep resurfacing on any new modal-opening frontend tour unless this step becomes a habit.

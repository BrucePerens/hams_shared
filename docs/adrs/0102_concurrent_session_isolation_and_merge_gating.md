...
**3. Branch protection is real, server-side, and equal for everyone, no exceptions:**
   - `enforce_admins: true` on both `hams_com` and `hams_shared` -- closes the exact hole this ADR exists
     to fix. No push, including an admin's, bypasses review or status checks again.

...
2. **Monitoring & Crash Recovery:** Do NOT continuously poll the MCP Watchdog yourself. You will be completely idle. Instead, instruct your "Monitor" sub-agent to do it for you.
   - **Monitor Prompt:** Instruct the Monitor to run in a continuous loop, calling the `wait_for_agent_state_change` MCP tool targeting BOTH Ignatz's conversation ID AND the Monitor's own conversation ID (so it watches itself for turn limits!). Use `stall_mins` set to 10, `max_wait_mins` set to 15, and `turn_warning_limit` set to 150. If the tool returns a normal state change (e.g., "sent a message"), the Monitor should just loop and call it again. If the tool returns a critical failure (e.g., "stalled/finished", "approaching turn limit", "gone"), the Monitor MUST instantly use the `send_message` tool to alert YOU (the Orchestrator).
...
**CRITICAL: AVOID GHOST SUBAGENTS.** Subagents frequently crash or finish silently without sending a message back to you. To monitor your reviewer subagents, you MUST use the MCP Watchdog (`wait_for_agent_state_change` tool). After spawning a batch of subagents, call `wait_for_agent_state_change` providing their conversation IDs in `target_agent_ids`, `stall_mins` set to 5, `max_wait_mins` set to 15, and `turn_warning_limit` set to 150. The tool will block until an event occurs. Continuously call this tool (across multiple turns) to monitor your subagents. 
